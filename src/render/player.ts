...
import { candelaFromNominalRange } from "../core/illumination.js";
import {
  distanceMetres,
    ...
    for (const member of this.stage.cast) {
      ...
      for (const { light, at } of member.lights.lit()) {
        lamps.push({
          ...
          // Rule 22's range, turned into the candela it was set from - Annex I, section 8.
          candela: candelaFromNominalRange(light.nominalRangeNauticalMiles),
    ...
    return [
      {
        ...
        candela: candelaFromNominalRange(ASSUMED_MARK.lightRangeNauticalMiles),
