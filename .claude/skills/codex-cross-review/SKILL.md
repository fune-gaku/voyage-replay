...
- **リポジトリの文書は丸ごと読ませず、見出しを見せて節を選ばせる。** 下の観点 1〜9 は
  蒸留版ではない——数えると `CLAUDE.md` の太字の罠は **148 件**、節は 23、
  `docs/domain-notes.md` の節は 53 で、9 件は **6%** にすぎない。渡さないと
  「diff を見ても違反していると分からない規則」を落とす。目次を読ませてから、
  diff が触っている節だけを読ませること（下の A に手順がある）
...
codex exec --model gpt-5.6-sol --sandbox workspace-write \
   ...
   **この 9 件は一覧ではありません。** このレポの罠は `CLAUDE.md`（太字の項目が
   148 件）と `docs/domain-notes.md` に節ごとに書いてあります。**全文は読まないで
   ください。** 代わりに、まず見出しだけを見て:
     grep -n '^###' CLAUDE.md
     grep -n '^##' docs/domain-notes.md
   diff が触っている領域に対応する節だけを読んでください（`sed -n '<開始>,<終了>p'`）。
   典型的には 1〜3 節で足ります。そこに書かれた規則に diff が反していないか見てください
   ——**diff の行だけを見ても「書かれた規則を破っている」ことは分かりません**。

   修正は絶対にしないこと。本文のファイル書き出しと verdict のみ。" \
  2>&1 | tee "$LOG"
```

**全文は渡さず、目次から節を選ばせること。** 丸ごと渡すと 1 反復あたり 5〜6 万トークンに
なり、見出しだけなら 76 行で済む。かといって渡さなければ、diff の行を見ても分からない規則
——「描画の帯域は解析の帯域と同じにする」のような——を落とす。冒頭の「費用について」を参照。
...
| `CLAUDE.md` / `domain-notes` を丸ごと読ませて 1 反復 5〜6 万トークン | 見出しだけ見せて節を選ばせる |
...
| モデルが CLI の既定に流れる | **`--model gpt-5.6-sol` を明示する。** このレポの 142 セッション（8 月以降）は全部これで、既定がたまたまそこへ解決していただけ——版が変われば動く。名前が拒否されたら `--model` を外して回し、何で回ったかを報告に書く |
