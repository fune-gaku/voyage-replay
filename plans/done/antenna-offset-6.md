...
- `src/ui/panels.ts` — 船の表に「hull offset」列。最接近の但し書き。
  **ここが一番書き直した**（下記）
...
Codex とのクロスレビュー 7 反復で出た指摘 10 件のうち **6 件が同じ形**だった——パネルの文章が、
実際に描かれたものより強い主張をしている。**この変更の主題（言うことと描くことを一致させる）が、
幾何ではなく散文の側で破れていた**わけで、次に UI 文言を足す人が読むべきはここ。

出た順に:

1. hull offset 列が offsets の有無だけで「補正した」と断言 → 方位が無ければ補正は当たらない
2. 最接近の但し書きが first の offsets だけを見て「アンテナ間」と断言 → `positionAt` が正本で、
   2隻で違うことがある。second だけが offsets を持つと但し書きごと消えるバグも同時にあった
3. 但し書きが「船体は offsets に載っている」と無条件に断言 → offsets が無い場合は
   *these offsets* が何も指さない
4. aspect 表の注記が周防灘事案の実測値を全シナリオに印字 → fixture の 2 隻は接触しない
5. ゼロ変位の offsets（アンテナが船体中心）を「移動した」と判定 → 算術をしたことは動かしたことではない
6. 部分的に方位が欠けるトラックで、CPA 時刻に関係なく「配置済み」と表示 → **配置は船の性質でも
   トラックの性質でもなく、その瞬間の性質**。レンダラは描画中の標本から判断している

**教訓は 1 つ: パネルの文は、描画側と同じ入力・同じ判断から組み立てる。** 今は
`isPlacedAt(prepared, epochSeconds)` が `sampleAt()` を引いて、`placementOf()` と同じ問いに
同じ答えを出す。表の列（トラック全体の要約）と但し書き（ある一瞬）で粒度が違うのは正しく、
同じページに「2/3 に適用」と「この瞬間はどちらも動いていない」が並ぶことがある。両方とも真。
